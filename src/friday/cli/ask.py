"""Single-shot ask command — one question, one answer."""
...
def run_ask(
    ...
    if mode:
        agent = create_agent(mode, settings, context)
    else:
        agent = create_router_agent(settings, context)

    try:
        deps.turn_stats.reset()
        with Status('Thinking...', console=console, spinner='dots'):
            result = asyncio.run(agent.run(question, deps=deps))
        record_turn_result(deps.turn_stats, result, settings.default_model)
